...
const checkFirstVisit = () => {
  ...
  const { favorite, currentFavorite } = cryptoDashData;
...
class App extends Component {
  ...
  validateFavorites = (coinList) => {
    ...
    this.state.favorites.forEach((favorite) => {
      if (coinList[favorite]) validatedFavorites.push(favorite);
    });
  ...
  loadingContent = () => {
    if (!this.state.coinList) return <div>Loading coins...</div>;
  ...
  filterCoins = (e) => {
    ...
    if (inputValue) this.handleFilter(inputValue);
    else this.setState({ filteredCoins: false });
  };
    ...
    for (let units = TIME_UNITS; units > 0; units--) {
      promises.push(
        cc.priceHistorical(
          ...
          ["USD"],
          moment()
            .subtract({ [this.state.chartIntervalTime]: units })
            .toDate()
        )
      );
    }
  ...
  prices = () => {
    ...
    this.state.favorites.forEach((sym) => {
      if (this.state.coinList[sym]) promises.push(cc.priceFull(sym, "USD"));
    });
    return Promise.all(promises);
  };
    ...
    const historical = [
      {
        ...
        data: res.map((coin, index) => [
          moment()
            .subtract({ [this.state.chartIntervalTime]: TIME_UNITS - index })
            .valueOf(),
          coin.USD,
        ]),
      },
